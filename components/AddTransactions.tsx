      ...
      <View className="flex-col gap-4 w-full p-3">
        ...
        <View className="bg-card rounded-xl p-2 mb-3">
          <View className="flex-row gap-2 items-center justify-center">
            <Pressable
              className={`flex-1 py-3 rounded-lg ${transactionType === "Income" ? "bg-primary" : "bg-card"}`}
              onPress={() => setTransactionType("Income")}
            >
              <Text
                className={`text-center text-lg font-medium ${transactionType === "Income" ? "text-foreground" : "text-zinc-400"}`}
              >
                Income
              </Text>
            </Pressable>
            <Pressable
              className={`flex-1 py-3 rounded-lg ${transactionType === "Expense" ? "bg-primary" : "bg-card"}`}
              onPress={() => setTransactionType("Expense")}
            >
              <Text
                className={`text-center text-lg font-medium ${transactionType === "Expense" ? "text-foreground" : "text-zinc-400"}`}
              >
                Expense
              </Text>
            </Pressable>
        ...
        <View className="flex-row flex-wrap justify-between mb-3">
          {categories.map((category, index) => {
            ...
            return (
              <Pressable
                key={index}
                className={`w-12 h-12 rounded-xl items-center justify-center ${isSelectd ? "bg-primary" : "bg-zinc-800"}`}
              >
                <Icons
                  name={category.icon}
                  color={"#ffffff"}
        ...
        <View className="flex-row flex-wrap justify-between mb-3 gap-2">
          {colors.map((color, index) => {
            ...
            return (
              <Pressable
                key={index}
                className={`w-12 h-12 items-center justify-center rounded-full border ${isSelected ? "border-foreground" : "border-transparent"}`}
